...
namespace ReaaliStudio.Utils.Extensions
{
    public static class EditorExtender
    {
        ...
        public static List<SerializedProperty> GetAllPropertiesOfType(SerializedObject serializedObject, Type type)
        {
            ...
            if (property.NextVisible(true))
            {
                do
                {
                    var propertyType = GetPropertyTypeFromReflection(property);
        ...
        public static T CreateScriptableAsset<T>(string assetName, string folderPath, bool ping = true) where T : ScriptableObject
        {
            T newSo = ScriptableObject.CreateInstance<T>();
            assetName = $"/{(string.IsNullOrEmpty(assetName) ? $"New{typeof(T).Name}" : assetName)}.asset";
        ...
        public static Type GetPropertyTypeFromReflection(SerializedProperty property)
        {
