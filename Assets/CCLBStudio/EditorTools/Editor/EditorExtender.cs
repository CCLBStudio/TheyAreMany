...
namespace CCLBStudio.EditorTools
{
    public static class EditorExtender
    {
        ...
        public static void DrawScriptField(MonoScript monoScript)
        {
            GUI.enabled = false;
            EditorGUILayout.ObjectField("Script", monoScript, monoScript.GetClass(), false);
            GUI.enabled = true;
            GUILayout.Space(5);
        }
        ...
        public static List<Type> GetAllConcreteTypes(Type type)
        {
        ...
        public static List<T> GetAllScriptsDerivingFrom<T>() where T : class
        {
            ...
            foreach (var guid in guids)
            {
                ...
                {
                    var method = typeof(EditorExtender).GetMethod(nameof(LoadScriptableAssets), BindingFlags.Static | BindingFlags.Public);
                    var genericMethod = method.MakeGenericMethod(monoType);
                    var instances = genericMethod.Invoke(null, null);
                    if (instances is IEnumerable<T> instanceList)
                    {
                        Debug.Log("Adding so instance list !");
                        result.AddRange(instanceList);
                    }
                }
                else
                {
